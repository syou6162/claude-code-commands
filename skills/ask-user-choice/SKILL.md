...
- **簡潔なheader**: 短いラベル（最大12文字）例：「ライブラリ」「アプローチ」
- **2-4個の選択肢**: それぞれにラベルと詳細な説明を付ける（**厳密に最大4個まで**）
- **multiSelect**: **デフォルトで`true`に設定**（柔軟性のため、ユーザーは1つまたは複数選択可能）

### 2. 充実した詳細な説明文を書く

説明文は長くてOK（300字以上でも問題なし）。情報量を充実させること。
...
1. **より速いユーザー対話**: タイプではなくクリック - 摩擦を大幅に削減
2. **より良い意思決定**: コンテキスト付きの充実した説明で情報に基づいた選択が可能
3. **改善されたUX**: 威圧的なテキスト入力ボックスではなく、構造化されスキャン可能な選択肢
4. **誤解の削減**: 事前定義された選択肢で共通理解を確保
5. **柔軟性**: multiSelect trueがデフォルトで、ユーザーは1つまたは複数を選択可能
...
- 詳細な説明を使う（300字以上でOK、コンテキストとトレードオフを追加）
- `\n\n`で段落区切りを使い、可読性を向上させる
- 柔軟性のため`multiSelect: true`をデフォルトにする
- 「その他」オプションは自動提供 - ユーザーは常にタイプ入力可能
- 関連する質問は1回に最大4つまで聞く
- 各選択肢のlabelは短く（1-5単語）、descriptionは充実させる
- **必要に応じて複数回質問する** - 4つ以上の質問がある場合やフォローアップが必要な場合は、最初の回答を得た後に再度AskUserQuestionを使用

❌ **してはいけないこと**:

- 1質問あたり4個を超える選択肢（ハード制限 - エラーになる）
- 1回の呼び出しで4つを超える質問（ハード制限 - エラーになる）
- 簡潔すぎる説明 - ユーザーは情報に基づいた意思決定のため詳細を求める
- 選択肢が真に相互排他的でない限り`multiSelect: false`を使う
- 「その他」が常に利用可能であることを忘れる - エッジケースを心配する必要なし
