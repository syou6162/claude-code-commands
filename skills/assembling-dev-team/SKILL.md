...
- **処理**: 指摘を振り分け（計画変更 → plan-updaterがプラン更新 / 実装修正 → 修正サイクル（3.2） / CI影響 → committer対応）
- **出力**: 指摘対応完了報告
...
指摘内容に基づき、フェーズ2と同様のサイクルで修正を実施（フェーズ2の指示フォーマットを再利用）:

1. reviewer が implementer に直接指摘を送付し、リーダーにも共有する（2.1.3と同様のフロー）→ implementer の修正完了報告を待つ
