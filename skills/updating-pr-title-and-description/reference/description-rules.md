...
**除外**: 開発日誌（esa.io）のURLは関連情報に含めない。esa.ioの開発日誌URLは内部の作業記録であり、PRの関連情報としては不適切。
...
関連するPRやIssueに言及する際は、「前タスク」「Phase 1」などの曖昧な表現ではなく、GitHubのフルURLを使用すること。リンクの前後には必ず半角スペースを入れる。

**良い例**:
```markdown
前タスクである https://github.com/owner/repo/pull/1234 で実装した認証基盤を拡張し、...
```
...
- **開発日誌（esa.io）のURLはPR説明文に含めない**: esa.ioの開発日誌URLは内部の作業記録であり、PR説明文の「関連情報」セクションにも記載してはいけない
