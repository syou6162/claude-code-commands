...
allowed-tools: Bash, Write, Read, Edit, TaskCreate, TaskUpdate, ExitPlanMode, EnterPlanMode, Task
model: sonnet
context: fork
---
...
2. ユーザーの承認を待つ
3. **承認された場合**: 手順2へ進む
4. **承認されなかった場合**: planモード中はEdit/TaskCreate/TaskUpdate等が使用できないため、**以降の手順を実行せず**処理を中断してユーザーに報告する
...
### 4. Taskツールでバックグラウンドレビュー実行

Taskツールで`run_in_background: true`を使ってsubagentを起動し、`codex review -`を実行してください。

**重要**: バックグラウンド実行により中間ログがmain agentのログに出ない

**出力先**: `.claude_work/codex_plan_review.md`（固定パス、既存ファイルは上書き）

<example>

**開発日誌がない場合のTaskツール実行例：**

```
Task tool:
  subagent_type: general-purpose
  run_in_background: true
  description: codex reviewでplanファイルをレビュー
  prompt: |
    ...
    出力は日本語で、具体的な指摘と改善提案を含めてください。" | codex review - | tee .claude_work/codex_plan_review.md
```

**開発日誌がある場合のTaskツール実行例：**

```
Task tool:
  subagent_type: general-purpose
  run_in_background: true
  description: codex reviewでplanファイルと開発日誌をレビュー
  prompt: |
    Bashツールで以下のコマンドを実行してください：
...
- Taskツールのバックグラウンド実行により、中間ログがmain agentのログに出ません
- 完了通知が来たらファイル（`.claude_work/codex_plan_review.md`）を読んで結果を確認してください
