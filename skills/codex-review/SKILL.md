...
allowed-tools: Bash, Write, Edit, Read, TaskCreate, TaskUpdate, Task
model: sonnet
context: fork
---
...
Task tool:
  ...
  description: codex reviewでコードレビュー
  prompt: |
    ...
    - planに記載された変更内容との整合性
    - コードの品質（可読性、保守性）
    - 潜在的な問題やバグ" | codex review - | tee .claude_work/codex_review.md
```
...
Task tool:
  subagent_type: general-purpose
  run_in_background: true
  description: codex reviewでコードレビュー（開発日誌含む）
  prompt: |
    Bashツールで以下のコマンドを実行してください：
    ...
    - コードの品質（可読性、保守性）
    - 潜在的な問題やバグ
    - 開発日誌に記載された開発指針との整合性" | codex review - | tee .claude_work/codex_review.md
```
...
- 完了通知が来たらファイル（`.claude_work/codex_review.md`）を読んで結果を確認してください
