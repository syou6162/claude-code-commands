...
### assembling-dev-team
「開発チーム集合」「チームで実装」「チーム編成して」の言及時に自動的に発動するスキル。プランファイルに基づいて実装・コミット・レビュー・プラン更新の4担当をAgent Teamsでスポーンし、ステップごとに実装→レビュー→コミットのサイクルを回します。リーダー（メインエージェント）がタスク管理・全体調整を行い、各メンバーが専門の役割を担当します。プラン更新担当はユーザーからの指摘やフィードバックをプランファイルに反映し、プランファイルをシングルソースオブトゥルースとして維持します。
